...
import string ## Para quitar los signos de puntuación
...
def load_input(input_directory):
    """Funcion load_input"""
    # glob.glob("files/imput*.txt") retorna una lista con los nombres de los
    files = glob.glob(f"{input_directory}/*.txt")
    
    ## Así se vería el código por extensión:
    """for file in files:
...
def line_preprocessing(sequence):
    ...
    # Una forma equivalente de hacerlo sería:
    # sequence = []
    ...
    ## O hacerlo directamente quitar los signos y convertir a minusculas:
    """
    sequence = [
    ...
    ## Eliminar mayusculas (convertir todo a minusculas):
    sequence = [
        (key, value.lower())
        for key, value in sequence
    ]
...
def mapper(sequence):
    ...
    return [
        (word, 1)
        for _, value in sequence # el value es la linea de texto
        for word in value.split()
    ]
    ## El _ guarda el último calculo hecho en python, en este caso, el "key"
...
def shuffle_and_sort(sequence):
    """Shuffle and Sort"""
    return sorted(sequence, key=lambda t: t[0])
    # recordar que lambda es una función anónima que se usa 
    # para hacer cálculos rápidos y sencillos, en este caso con t de tupla
...
def reducer(sequence):
    ...
    return [
        ...
        for key, group in groupby(sequence, key=lambda t: t[0])
    ]
    ...
    for key, value in sequence:
        if key in diccionario:
            ...
            diccionario[key] = value ## pero no me sirve el diccionario
    return list(diccionario.items()) # me retorna una lista de tuplas
    """
...
# file/input/*.txt ---> retorna todos los .txt
